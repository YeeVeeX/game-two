# SPARK sesión 36 — harvest-first support; the named-refusal promotion chain is COMPLETE (no J1 candidate — never manufacture one); shared-save first crossing still loaded; v19 waits at the brainstorm
...
## Program state (2026-08-22 session-35 close — verify live)
...
- v18 CLOSED · **v19 NOT open** (brainstorm at the owners' word; agenda:
  ...
  companion/ally AI).
- **s35 was a ZERO-DELTA docs-only close** (`498b87e`): gate verified on
...
Baselines at staging: origin/main tip = the s36 spark commit — `git log
--oneline -4` reads: that commit · `498b87e` s35 docs · `49b6b5d` s35
spark · `af1c681` s34 docs. Save `saves/world.json` md5
...
- **A live coop session = lag segment S1**: support + harvest verbatim
  per `drafts/_lag-t2-evidence/README.md`; BOTH seats at the s36 tip
...
`drafts/_s36-live-harvest-<date>.md`: log md5 + `TELEMETRY persist`
...
Checkpoint entry (s36) · docket rows where verdicts landed (pointers) ·
AGENTS one-liner ONLY if a verdict landed · commit docs + push. Honest
no-delta lines for whatever stayed pending. Author the s37 spark at
...
One attended session ~1-2 h (shorter than s31–s34: no default J1; s35
precedent — a zero-delta session closes in well under an hour).
