...
## YOUR AREA — (a) sim primitives + boss block + save/progression/netplay

Paths: `src/game/**` (world.rb, pack.rb, stations.rb, save_state.rb, progression.rb,
transients.rb, telemetry.rb, crossing.rb, camera.rb, creature/enemy/threat files, boss
block), `src/core/**` (event_bus, data_store, tile_map, input), `src/net/**`
(fingerprint, protocol, session, lockstep), `data/balance/**`, `data/netplay.json`,
`tools/a3_stream_diff.rb`, `tools/a3_leash_trace.rb`, and the tests under `test/game/`,
`test/core/`, `test/net/`, `test/harness/sim_identity_canary_test.rb`.

Focus questions: (1) does any sim rule added in the range read presentation state,
wall-clock, Float, or per-machine data (lockstep risk)? (2) `World#digest_snapshot` —
does every new sim fact enter it, and does any presentation-only field leak into it?
(3) MUNDO VIVO transitions/seals/boss block: refusals NAMED, data-driven, events
registered? (4) ally brain (`threat.json ally.*`, `World#ally_*`): is the OFF path
byte-inert as claimed (read the code path, not the claim)? (5) `save_state.rb`: does
`apply!` order + `home_refusal` + `envelope_refusal` still hold for every key; any path
that writes the live save outside clean quit? (6) `world.rb` is at 1776/1800 — name the
extraction seams a refactor should cut (plain objects), as MINOR findings.
