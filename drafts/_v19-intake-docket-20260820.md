...
|---|---|---|---|
| J-1 Tibia stationary facing (Ctrl+direction) | `drafts/_junior-v19-ideas-20260819.md` idea 1 | sim (input+protocol) | **CLOSED 2026-08-21**: shipped v18-era (`28017d8`); Junior re-tested post-pull — "FUNCIONA" (`drafts/_junior-pilot-walk-20260821.md`, f60d51e); the earlier "segue desativado" adjudicated as pre-pull mixed build. |
...
| Junior solo finding A — ally-AI acquisition gating (allies charge when minion enters THEIR vision) | `drafts/_junior-solo-playtest-findings-20260820.md` ACHADO 1 + his same-night CORREÇÃO (verbatim, allies-as-purchasable-resource reading RESTORED) | sim (AI) | Seat reading marked as reading, not verdict; telemetry-corroborated. Ties verdict row 6. Design debate at brainstorm. **Mechanism CONFIRMED read-only 2026-08-21** (`9a7dd98`: ONE AiController serves all factions, controllers.rb:100; allies only ADD possessed-follow + seat-gated flee) — player hypothesis was exact; R-A3 stays FROZEN until the brainstorm. |
...
| TOWN 1 content v0 — no revive/level stations, two empty rooms | `drafts/_junior-primeira-travessia-20260821.md` §Depoimento (first-crossing bank) + zone data verified (zone_7 stations = bank + seal only; camp = bank/altar/vat) | data (zone content) / design | BANKED brainstorm input 2026-08-21; rhymes with finding B (deep-zone banking). Design v0 vs content gap = the owners' call. |
...
| Lag T4 — vsync release result | `drafts/_lag-t4-vsync-20260820.md` §6 verdict + Junior's `2e23bd3` (log md5 `1186fe4a…`) | app/presentation (pacing) | **ADJUDICATED s26-close: REFUTED on the decisive 59 Hz seat** (p50 16,8→16,7 — Gosu's update_interval timer owns cadence, not the swap); ticket CLOSED no-ship-by-default, flag stays a diagnostic lever. |
| Lag — the ~7% in-process long-frame tail | `drafts/_lag-t3-verdict-20260820.md` §4 + Junior's T4 probe (`drafts/_junior-pilot-walk-20260821.md` §ACHADO: over20 16.8%, draw p95 15.3 ms) | app/perf | **CLOSED 2026-08-21 (s32)** — trigger fired (Junior FELT it + banked numbers) → s28 diagnosed DRAW as the tail and shipped the merged-static-runs render fix (`dd8ff40`, captures byte-identical; ticket `drafts/_wb-t5-wirein-20260821.md`); **re-measure VERIFIED on the decisive seat** (`drafts/_junior-remeasure-20260821.md`: same T4 route + scratch, draw p95 15.3→8.5 ms, over20 16.8%→5.15%, period p90 33.4→17.5 ms, feel "muito mais leve") — pre-declared class 1 CLOSES. Leftover named, unbuilt: motif-strip authoring (DATA-class, v19 candidate on owner word behind its own Rule-2 gate + wall; texture/macro composition DEAD by the float-camera law). Caveat: over100 3→9 isolated spikes (zone-load/window-focus suspected), not the continuous tail. |
| Basement ambience (Junior's walk question: "silêncio de design ou lacuna?") | `drafts/_junior-pilot-walk-20260821.md` §ear-checks + T4 ticket rider 8 (`drafts/_wb-t4-pilot-20260821.md`) | data (audio) | ANSWERED: design v0 — basements ship nil-key silent by T4 rider 8; when an owner bed render exists it becomes ONE data row (`ambience_<key>`), a future cue-spec mail candidate (the sent T3 mail stays FROZEN). |
| Lobber shot passes through enemies (Junior, player-reported) | `drafts/_junior-solo-playtest-findings-20260820.md` §ACHADO NOVO (mechanism read-only: projectile.rb:73 exact-tile hit test at landing frame vs step-start tile commit) | fork: presentation-legibility vs sim-class hit-test change | v19 brainstorm names the class; same legibility family as the enemy-strike-tiles fix; zero code owed now. |
| BOSS-1 dread iteration | PARKING_LOT.md ("OPEN-FOR-EXPOSURE, zero code owed") | design/exposure | **FIRST ORGANIC EXPOSURE BANKED 2026-08-21** (`drafts/_junior-primeira-travessia-20260821.md`: varekka engaged=1 chants=1 interrupted=0 slain=1, chant expired; player quotes — undodgeable arena-wide skill · boss dies in few hits · return fight = 1x1, minions passive). Stays OPEN-FOR-EXPOSURE, zero code owed — design input for the owners. |
