...
3. Precisa mudar um arquivo fora de `owns`? NAO toque. Escreva um PATCH REQUEST no SEU
   receipt `drafts/lanes/receipts/s4-equipment.md` (arquivo, chave/linha, valor exato, por que).
...
7. Ao terminar: suite verde, `lane_guard` OK, `git push -u origin lane/s4-equipment`, e o receipt em
   `drafts/lanes/receipts/s4-equipment.md`: `RECEIPT: s4-equipment <sha> READY <resumo de 1 linha>` (+ PATCH REQUESTS).
