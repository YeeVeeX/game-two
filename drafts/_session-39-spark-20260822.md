# SPARK sesión 39 — harvest-first support; the named-refusal promotion chain is COMPLETE (no J1 candidate — never manufacture one); shared-save first crossing still loaded; v19 waits at the brainstorm
...
(no design fork is open); paid calls 0 by default — ONE named exception:
any override/defect lane that ships a gated change owes its wall gates.
Evidence-first: claims are not evidence — file:line, log line, or
UNVERIFIED tag. Bot lines are never fun/oracle evidence.

## Program state (2026-08-22 session-38 close — verify live)
...
- v18 CLOSED · **v19 NOT open** (brainstorm at the owners' word; agenda:
  ...
  companion/ally AI).
- **s35 (`498b87e`), s36 (`58a82ce`), s37 (`2979079`), and s38
  (`e525723`) were ALL ZERO-DELTA docs-only closes** — four
  consecutive: gate verified on every surface (no owner play, no Junior
  banks, no mail), nothing landed, nothing shipped — the correct
  outcome under the ordering law. The world is waiting on its humans,
  not on engineering.
- **The last shipped engineering change is s34's seal GATING law**
  (`abe04d6`, grilled — verdict TIGHTEN; reviewed PASS-WITH-NITS): a
  seal's `opens` must name a transition with TRUTHY `sealed` — unsealed
  or `requires_defeats`-only ways refuse NAMED at zone load. Grill
  answers + 6-reader breach-fact census banked:
  `drafts/_s34-seal-gating-20260821.md`. Suite at 1022.
- **The named-refusal promotion chain (s31→s32→s33→s34) is COMPLETE.**
  No once-RECORDED engineering candidate remains: s33 nit 2 = cosmetic
  churn (refused), s34 finding 5 = no divergence to close (refused),
  everything else v19-class or owner-paced. **This session has NO
  default J1 — do not manufacture one.** If nothing landed, verify the
  gate, close honest and CHEAP (docs-only if literally nothing moved).
- **THE FIRST HUMAN GATE CROSSING LANDED at s32** (Junior, HIS solo
  live save, `8478cde`). **The SHARED save on THIS machine is UNMOVED**
...
Baselines at staging: origin/main tip = the s39 spark commit — `git log
--oneline -4` reads: that commit · `e525723` s38 docs · `10d637f` s38
spark · `2979079` s37 docs. Save `saves/world.json` md5
...
logs **40×2** (`$TEMP` + `/tmp`, pattern `game_two_session_*.log`;
newest 08-21 01:39 — already counted) · seat mail at `~/.pi/agent/mail/`:
`game-two/` inbox EMPTY done/=22, `game-two-audio/` holds the unanswered
T3 cue-spec (FROZEN — never reply-ask) · tmp/soak newest
`20260820-232208` · untracked `drafts/_refs/` only ·
`tmp/pilot_walk/world.json` = local scratch world (md5 `76881fd7…`,
sessions=1 banked=20 home=zone_7; never delete, never play it) · coop
S1 evidence dir `drafts/_lag-t2-evidence/` = README only. Known ignored
leftovers (NOT deltas): `captures/map/world_41a9683d_*.png` ·
`tmp/s3?_review_*.txt` (s30–s34) · `tmp/s31_diff.txt` ·
`tmp/s34_diff.txt` · `tmp/s30_review_out.txt`.
...
bank) → J2/J3 harvest+adjudicate (docs-only, fast). If a harvested log
shows a crossing landed WRONG (bad tile, wrong zone, crash at a gate),
that defect's reproduction lane opens (classify + owner chat BEFORE
fixing; wall gates owed if live-zone bytes or visual surfaces move).
No deltas → verify gate → docs close, stop — a short honest session is
...
- **A live coop session = lag segment S1**: support + harvest verbatim
  per `drafts/_lag-t2-evidence/README.md`; BOTH seats at the s39 tip
...
`drafts/_s39-live-harvest-<date>.md`: log md5 + `TELEMETRY persist`
...
Checkpoint entry (s39) · docket rows where verdicts landed (pointers) ·
AGENTS one-liner ONLY if a verdict landed · commit docs + push. Honest
no-delta lines for whatever stayed pending. Author the s40 spark at
...
- Cross-repo: read siblings freely, write ONLY via seat mail at
  ...
  seat. Sub-agent prompts forbid seat-mail handling ("touch NOTHING,
  including seat mail"); audit the mail dir after every sub-agent run.
...
- **Sim-class anything** (tile behaviors, spawn logic, AI, balance,
  economy, OR-gate seal semantics) → RECORDED for v19, refuse in
  writing.

## Budget + stop conditions

One attended session ~1-2 h (shorter than s31–s34: no default J1;
s35–s38 precedent — a zero-delta session closes in well under an hour).
Council 0. Paid calls 0 — the ONLY exception is an override/defect lane
shipping a gated change. Sub-agents: a fresh-eyes reviewer ONLY if a
defect lane ships code, nothing else. Context guard 85% →
compact-checkpoint skill.
**Stop when:** any landed deltas adjudicated/harvested, checkpoint +
push. No deltas = verify gate, docs close, stop — SHORT IS CORRECT.
**Stop early, honestly, if:** the owners redirect (their word routes) ·
the brainstorm opens (it takes over) · any gate fails non-mechanically
(STOP, classify in writing) · a sim-class ask arrives without the
brainstorm (refuse in writing, RECORD for v19).
