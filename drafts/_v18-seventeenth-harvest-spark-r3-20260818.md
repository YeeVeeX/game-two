# SPARK: v18 session 10 — the SEVENTEENTH: full harvest + adjudication (or honest PARTIAL/EMPTY); solo chain links + v19 intake ride
...
ask, revision r3 — supersedes r2
(`drafts/_v18-seventeenth-harvest-spark-r2-20260818.md`). What changed
since r2: the world MOVED by a legitimate owner solo session (chain
link #1, banked + committed), so the chain anchor is now a MOVING
expectation; the live-launch protocol (dev seat opens the game for the
owner mid-session) is now codified below. If both ritual sessions +
all eight answers exist, you harvest, run the four Half-A checks,
record Half B verbatim, walk EVERY routing row, and decide on the
spec's own terms. If evidence is incomplete, you bank what exists
verbatim, name the gaps, and STOP — session 5 (`72e9297`) is the
PARTIAL precedent; session 9 (`2db6118`/`5850df1`) is the EMPTY +
solo-link precedent. Never fudge, never waive, never nag the humans
to play.
...
2. `docs/CHECKPOINT.md` — top THREE entries (session 9: EMPTY gate,
   solo chain link #1 addendum, new anchor; session 8: soak shipped +
   residue laws; session 7/5 for the banking format).
3. `drafts/_v18-fun-verify-skeleton-20260818.md` — THE working file:
   ...
   questions, residue traps (tmp/netplay + soak), the session-9
   re-check block, **Solo chain link #1** (the banking pattern you
   reuse for any new solo log), side-signal HELD, gaps list. You fill
   THIS file; adjudication sections get written only in FULL mode.
...
Baselines at session-9 close (2026-08-18 ~19:30):
...
- **Save expectation IF NOTHING MOVED IT:** play-path strict decode
  (real `Core::DataStore` + `App::SaveStore#load`) returns LOADED
  `digest=602e94bbf7d417d845c73e3702fd4675`, sessions=3, banked=20,
  seals=2, marks=3, boss_1_defeats=1; file md5
  `213076c540cc9eed846172748aae2e10`, mtime 2026-08-18 18:00:31.
  Owner solo/coop play legitimately moves it — but **every save move
  must have a matching human launcher log to bank; a moved save with
  NO matching log is a NAMED anomaly** (investigate, never assume).
  The save is DIAGNOSTIC context for the chain walk, never one of the
  four checks.
- **Junior baseline:** no commits past `5850df1`; stale remote
  branches (`origin/junior/ci` 2026-08-16, `origin/v13-aoe`) stay
  stale. His artifacts arrive by paste, drafts file, or commit only —
  his ritual logs live on HIS machine.
...
- Also inventory NEW `tmp/soak/*/report.txt` newer than 2026-08-18
  15:16 (the owner has the overnight one-liner) → feeds Job 5, never
  the oracle.
...
- **EMPTY** = nothing new → dated re-check line appended to the
  skeleton's gate-result section (the session-9 block is the
  pattern), then Job 5+.
...
New `tmp/soak/*/report.txt` runs (newer than 2026-08-18 15:16): read
reports + any FAIL bundles. Findings are RECORDED with next-spark
