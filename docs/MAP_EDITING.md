...
|---|---|---|---|
| `zone_7`, `basement_1`, `basement_2`, `dungeon_1` (the "pilot four"), plus `district`, `district_two`, `low_quay`, `dungeon_2`, `dungeon_3`, `dungeon_4`, `ember_1`, `ember_2`, `ember_3` (MUNDO VIVO, re-authored through the same door) | LDtk importer emissions — the ZONES list in `test/tools/pilot_authoring_test.rb` is the source | edit `authoring/pilot.ldtk` (+ per-zone sidecar `authoring/<zone>.sidecar.json`: palette, decor, gradient) → re-import via `tools/import_ldtk.rb` → deliberate copy into `data/zones/` | **NEVER hand-edit the JSONs** — `test/tools/pilot_authoring_test.rb` byte-compares committed JSON against a fresh emission; any drift = red suite. Render-only decor/palette fixes are SAFE class and still go through the sidecar (E1c s137 precedent: dungeon_1's pre-swap water decor) |
...
| `camp` (HUB 1), `nest`, `gate_fixture`, `grass_fixture`, `slow_door` | hand-authored JSON (pre-WB era) | direct JSON edit, one concern per commit | strict loader refuses malformed zones NAMED at load; suite + gates still bind |
