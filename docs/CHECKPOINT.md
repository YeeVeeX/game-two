...
CLAIMED: MUNDO VIVO (v21 proposal) — Junior seat, branch `junior/mundo-vivo` (restore tag `restore/pre-mundo-vivo-20260904` = `cf9a6a6`). **FASES 0–6.5 EXECUTADAS no branch (s126–s127):** plan · art layer · ambience · tile faces · 6 sim primitives (spread/petrify/blink/charge/beam/poison) · boss block (BOSS 1 byte-identical, BOSS 2 + BOSS 4 phased) · **THE SWAP** (MEDUSA LOWER → DUNGEON 1; MUSGO A → ZONE 5 with BOSS 1 + spore fauna; L9 retired-seal migration) · **the MEDUSA TOWER complete** (dungeon_2/3/4 = Junior's A/B/C patterns, forced loops 2.19×/2.84×/2.80×, BOSS 2 at the bottom, rungs 8/10/12, cap 15→18). Evidence: `drafts/_junior-mundo-vivo-f{1..6}-20260905.md` (branch). Merge to main ONLY after the full wall sweep is banked (FASE 1 sweep 34/37 at write time; a 1+2+3+6 sweep is owed) + tower/moss sentinels authored (tuner). **NINETEENTH CLOSED — CUMPLIDO WITH NAMED ITEMS (s125); ALL FREEZES LIFTED.** Owner-pending (never nag): MUNDO VIVO ratification (plan D1–D9; D1 has a number; D3/D5/D7 executed on dev recommendation — reversible) · swap-spec 3 ratifications (D-HOLE reversal executed; internal seal retired w/ named migration; BOSS 1 not duplicated) · totem COEXISTENCE word.
