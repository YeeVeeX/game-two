...
CLAIMED: **v22 GRILL "THE PRICED DEATH" + contract hygiene + art-lane charter — Gabriel seat, s131, 2026-09-05 (spark: `docs/sparkups/sparkup-v22-grill-priced-death-merged-20260905.md`; Phase 0 hygiene commits first, then the owner-live grill; docs + tools/harness/test only, no src/ or data/ touches).** Last closed: **WB-T6 LDtk foundation wave CLOSED DONE (s130, 2026-09-05)**; before that: **MUNDO VIVO (v21) LANDED ON MAIN `476ee32` (s128, 2026-09-05 ~07:00)** after the full wall. Restore tags: `restore/pre-mundo-vivo-20260904` (`cf9a6a6`, before) · `restore/post-mundo-vivo-20260905` (the merge). Next for either seat: play it (`bin\play.cmd pt-br` / `bin\play.cmd es` — ZONE 7's hole → the tower at lvl 6/8/10/12; the south-meadow hole → BRASA at 13/15/17; ZONE 4 → the moss at floor -3). Owner-pending (never nag): Gabriel's ratification of the plan (D1–D9), the swap's 3 decisions, totem COEXISTENCE word, and the FASE 7 city proposal (both peers). Named debts: `vat_economy` 2 rows + `aoe_specials challenge_reads` (re-author) · basement_3 · D5 · `pool` · `varekka_duel`/`burn_duel` re-author on the moss vault.
