...
CLAIMED: TS totem re-work (cadence 3 s, radius +2, heal scales with hp pool) — Gabriel seat, s138. Files: `src/game/stations.rb` (`tick_totems!`), `data/balance/sustain.json`, `data/display.json` (`totem_pulse_frames` only if the reel says so), `test/game/totem_test.rb`, `harness/scripts/totem_pulse.json` + its `gate_checks.json` row, `test/harness/sim_identity_canary_test.rb` (rebank only if the audit says so). world.rb untouched. Spark: `docs/sparkups/sparkup-v22-ts-totem-20260906.md`.
