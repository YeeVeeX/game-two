...
EOF
echo "SOAK RUN $RUN mode=$MODE n=$N ticks=$TICKS seed_base=$SEED timeout=${TIMEOUT_S}s"
...
for i in $(seq 1 "$N"); do
  ...
  echo "EP$i port=$PORT host_seed=$HSEED joiner_seed=$JSEED pids=${HPID:--}/${JPID:--}"
