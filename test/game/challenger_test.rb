...
class ChallengerTest < Minitest::Test
  ...
  def seize_possessed!
    ...
    assert_equal challenger, world.possessed.seized_by
  end

  # --- chant ---------------------------------------------------------------

  def test_chant_starts_in_range_and_he_stands_still
    face_challenger!(dist: 5)
    started = collect(:challenger_chant_started)
    drive(world, scripted({}), 3)
    assert_equal 1, started.length
    assert challenger.chanting?
    post = challenger.tile
    drive(world, scripted({}), 30)
    assert_equal post, challenger.tile, "pronunciation is stillness"
  end
  ...
  def test_chant_left_is_readable_and_counts_down
    ...
    drive(world, scripted({}), 3)
    assert challenger.chanting?
    before = challenger.chant_left
    assert_operator before, :>, 0
    drive(world, scripted({}), 10)
    assert_operator challenger.chant_left, :<, before
  end

  def test_chant_does_not_start_out_of_range
    face_challenger!(dist: SEIZE[:range_tiles] + 3)
    started = collect(:challenger_chant_started)
    drive(world, scripted({}), 10)
  ...
  def test_damage_interrupts_the_chant_and_buys_the_cooldown
    ...
    refute challenger.chanting?
    assert challenger.seize_cooldown.positive?, "interrupting buys the room ten seconds"
  ...
  def test_chant_pins_the_body_at_start_not_the_echo
    ...
    drive(world, scripted({}), SEIZE[:chant_frames])
    assert_equal challenger, pinned.seized_by, "the pinned BODY answers, not the possessing player"
  ...
  def test_seizure_walks_the_possessed_to_him_and_suppresses_movement
    ...
    dist = [(body.tile[0] - challenger.tile[0]).abs, (body.tile[1] - challenger.tile[1]).abs].max
    assert dist <= 1, "the body stands AT him against held input (got dist #{dist})"
    assert_equal challenger, body.seized_by, "still held (450f duration)"
  end
  ...
  def test_swap_escapes_even_staggered
    ...
    assert_equal challenger, body.seized_by, "the abandoned flesh still answers the name"
  end
  ...
  def test_seizure_expires_and_survives_distance
    ...
    (world.pack.living - [world.possessed]).each do |m|
      m.take_hit(damage: 9_999, attacker: challenger)
    end
    seized = collect(:vessel_seized)
    ...
    challenger.walker.teleport(23, 49)
    ...
    # time stopped.
    assert_operator challenger.seize_cooldown, :>,
  ...
  def test_his_death_ends_the_seizure
    seize_possessed!
    ended = collect(:seizure_ended)
    challenger.take_hit(damage: 9_999, attacker: world.possessed)
    drive(world, scripted({}), 2)
    assert_equal 1, ended.length
  ...
  def test_seized_body_death_ends_exactly_once_even_through_the_wipe
    ...
    world.pack.living.each { |m| m.take_hit(damage: 9_999, attacker: challenger) }
    drive(world, scripted({}), 2)
  ...
  def test_zone_exit_clears_the_seizure
    ...
    drive(world, scripted({}), SEIZE[:chant_frames] + 3)
    pinned = world.possessed
    assert_equal challenger, pinned.seized_by
    ended = collect(:seizure_ended)
  ...
  def test_he_never_respawns
    ...
    challenger.take_hit(damage: 9_999, attacker: world.possessed)
    drive(world, scripted({}), 2)
    assert_nil challenger, "one man, one death"
    drive(world, scripted({}), 500)
    assert_nil challenger, "no respawn record was ever scheduled"
  end
end
