...
#      carries the pack low_quay [24,34] -> zone_7 (FASE 6.1: the MUSGO south door), landing on the declared
...
#   3. the return zone_7 [1,14] -> low_quay [24,33] is FREE (no fact required).
...
class OpenGateCompositionTest < Minitest::Test
  ...
  def test_the_earned_defeat_opens_the_gate_on_the_real_pair
    ...
    w.start_in("low_quay")
    w.possessed.walker.teleport(24, 34)
    park_allies_adjacent(w, [24, 34])
    drive(w)
  ...
  def test_the_return_is_free
    ...
    assert_equal [24, 33], w.possessed.tile,
